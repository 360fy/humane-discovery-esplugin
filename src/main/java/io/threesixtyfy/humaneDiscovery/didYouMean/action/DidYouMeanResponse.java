...
public class DidYouMeanResponse extends ActionResponse implements StatusToXContent {
    ...
    private DidYouMeanResult[] results;
...
//        this.shardFailures = shardFailures;
    ...
    public DidYouMeanResult[] getResults() {
        return results;
    }
    ...
    public XContentBuilder toXContent(XContentBuilder builder, Params params) throws IOException {
        ...
        builder.startArray();
        for (DidYouMeanResult hit : results) {
    ...
    public static DidYouMeanResult readDidYouMeanResult(StreamInput in) throws IOException {
        DidYouMeanResult result = new DidYouMeanResult();
        result.readFrom(in);
        return result;
    }
    ...
    public void readFrom(StreamInput in) throws IOException {
        ...
        if (resultsSize == 0) {
            ...
            results = new DidYouMeanResult[resultsSize];
            for (int i = 0; i < results.length; i++) {
                results[i] = readDidYouMeanResult(in);
            }
        }
    ...
    public void writeTo(StreamOutput out) throws IOException {
        ...
        if (results.length > 0) {
            for (DidYouMeanResult result : results) {
                result.writeTo(out);
            }
        }
