...
public class IndexEventListenerImpl implements IndexEventListener {
    ...
    public void afterIndexCreated(final IndexService indexService) {
        if (!indexService.getIndexSettings().getSettings().getAsBoolean(TokenIndexConstants.TOKEN_INDEX_ENABLED_SETTING, false)) {
            return;
        }
        ...
        sharedChannel.getIndexOperationsQueue().add(new TokenIndexCrudRequest(TokenIndexCrudRequest.RequestType.CREATE, indexName));
    }

    @Override
    public void afterIndexDeleted(final org.elasticsearch.index.Index index, final Settings indexSettings) {
        if (!indexSettings.getAsBoolean(TokenIndexConstants.TOKEN_INDEX_ENABLED_SETTING, false)) {
            return;
        }
        ...
        sharedChannel.getIndexOperationsQueue().add(new TokenIndexCrudRequest(TokenIndexCrudRequest.RequestType.DELETE, indexName));
    }
